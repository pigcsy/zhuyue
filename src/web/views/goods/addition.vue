        ...
        <el-container direction="vertical" v-loading="isSubmit">
            <el-header>
                <div id="sign">
                    ...
                    <span class="signtext">新增产品 :: 贷款产品</span>
                </div>
                <div id="group">
...
export default {
    ...
    methods: {
        ...
        submit (formName) {
            ...
            that.$refs[formName].validate((valid) => {
                if (valid) {
                    that.isSubmit = true; 
                    setTimeout(() => {
                        Goods.addition({ ...that.data }).then((res) => {
                            if (res) {
                                if (res.code === 10001) {
                                    that.$message({ center: true, type: "success", duration: "1500", message: "保存".concat(res.result), onClose: () => that.close()});
                                } else {
                                    that.isSubmit = false; 
        ...
        close () {
            const that = this;
            that.$router.replace({ "path": "/backend/goods" });
        },
        /**
         * 上传成功
         */
